...
### 1. Обработка файлов для первого и дополнительных авторов (SharedTagProcessor class).
...
### 3. Блоковая обработка шаблонов (BlockProcessor class).
...
Пользователи могут редактировать подсказки для тегов, которые отображаются в текстовых полях при генерации документов. Это позволяет стандартизировать и упростить ввод данных.

Как использовать:
Переход в режим редактирования
На стартовом экране (ViewModelStartScreen) нажмите кнопку "Редактировать подсказки". Это откроет панель ViewModelTextFields в специальном режиме.

Загрузка тегов

Выберите файлы (doc/docx), чтобы извлечь теги из них.
Либо используйте теги из базы данных (если файлы не выбраны).

Изменение подсказок
Для каждого тега отображается текстовое поле:
Если подсказка уже задана, она будет предзаполнена.
Если подсказка отсутствует, отображается placeholder: "Введите подсказку для ${тег}".

Сохранение изменений
После внесения правок нажмите кнопку "Сохранить в БД". Подсказки сохраняются в базе данных и будут автоматически использоваться при генерации документов.

Особенности:
В режиме редактирования можно работать как с новыми тегами из файлов, так и с существующими из базы данных.

Подсказки поддерживают многострочный ввод и форматирование.

При генерации документов пользователи видят сохранённые подсказки вместо стандартных placeholder-ов, что ускоряет заполнение полей.

База данных с подсказками хранится C:\Users\User
...
В рамках тестирования выполнен анализ времени генерации документов с помощью JUnit. Тестирование проводилось на программе, генерирующей файлы `.docx`, содержащие абзацы, теги и таблицы, при этом изучалось влияание объема данных на процесс генерации.
...
| **Тип файла** | **Слов в файле** | **Количество файлов** | **Тегов на файл** | **Среднее время на файл (сек.)** | **Среднее время с конвертацией в PDF (сек.)** |
|---------------|------------------|-----------------------|-------------------|----------------------------------|----------------------------------|
| **Small-1500**   | 2 000            | 100                   | 1 500               | 0,07305                            | 2,83005                         |
| **Medium-1500**  | 10 000           | 50                    | 1 500             | 0,09935                           | 3,16758                         |
| **Huge-1500**    | 50 000           | 10                    | 1 500             | 0,12864                           | 7,06854                         |
| **Medium-2600**  | 10 000           | 50                    | 2 600             | 0,21539                           | 3,27142                         |
| **Huge-2600**    | 50 000           | 10                    | 2 600             | 1,37381                           | 8,51631                         |
| **Medium-5500**  | 10 000           | 50                    | 5 500             | 1,61678                           | 5,07708                         |
| **Huge-5500**    | 50 000           | 10                    | 5 500             | 9,95829                           | 17,25088                         |
...
Анализ данных из сводной таблицы тестовых данных демонстрирует, что на время генерации документов существенно влияет как количество тегов, так и объем текста в файле.

- **Влияние количества тегов:**  
  При сравнении файлов с одинаковым объемом текста, увеличение числа тегов приводит к резкому росту времени генерации. Например, для файлов с 50 000 словами среднее время генерации возрастает с 0,12864 сек. (Huge-1500) до 9,95829 сек. (Huge-5500). Аналогичная тенденция наблюдается для файлов с 10 000 словами, увеличивается время с 0,09935 (Medium-1500) до 1,61678 (Medium-5500) сек.

- **Влияние объема текста:**  
  При фиксированном количестве тегов увеличение объема текста оказывает менее значительное влияние. Например, для файлов с 1 500 тегами среднее время генерации возрастает с 0,09935 сек. (для 10 000 слов: Medium-1500) до 0,12864 сек. (для 50 000 слов: Huge-1500).

- **Конвертация в PDF:**  
  Время для конвертации включает в себя время генерации документа. Зависимость наблюдается только от количества слов в файле, это видно по разнице между временем конвертации и временем генерации:

```
Для Huge-1500: 7,06854 сек. (PDF) − 0,12864 сек. (генерация) ≈ 7 сек.
Для Huge-2600: 8,51631 сек. (PDF) − 1,37381 сек. (генерация) ≈ 7 сек.   
Для Huge-5500: 17,25088 сек. (PDF) − 9,95829 сек. (генерация) ≈ 7 сек.
```

Можно сделать вывод, что основным фактором, определяющим время генерации, является количество тегов. При этом объем текста оказывает второстепенное влияние.
